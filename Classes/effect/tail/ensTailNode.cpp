...
bool CtailSprite::init(const string&texFileName)
{
	...
	{
        GLchar * vertSource = (GLchar*) CCString::createWithContentsOfFile(CCFileUtils::sharedFileUtils()->fullPathForFilename("res/BlackPink/shaders/tailEffect.vsh").c_str())->getCString();
		GLchar * fragSource = (GLchar*) CCString::createWithContentsOfFile(CCFileUtils::sharedFileUtils()->fullPathForFilename("res/BlackPink/shaders/tailEffect.fsh").c_str())->getCString();
...
void CtailSprite::update(float dt){
    //add a pos
    Point posCur=this->getPosition();
    if(m_footPrintList.empty()){
        m_footPrintList.push_back(CfootPrint(posCur,m_initialOpacity));
    }else{
        Point posFoe=m_footPrintList[(int)m_footPrintList.size()-1].m_pos;
...
Cmesh* CtailSprite::generateMesh(const vector<CfootPrint>&footPrintList){
    ...
    for(int i=0;i<nFootPrint-1;i++){
        const Point&start=footPrintList[i].m_pos;
        const Point&end=footPrintList[i+1].m_pos;
    ...
    for(int i=0;i<nFootPrint;i++){
        if(i==0){
            ...
            Point posf=footPrintList[i-1].m_pos;
            Point pos=footPrintList[i].m_pos;
            Point posn=footPrintList[i+1].m_pos;
            Point vec=pos-posf;
            Point vecn=posn-pos;
    ...
    for(int i=0;i<nFootPrint-1;i++){
        ...
        //
        Point start=footPrintList[i].m_pos;
        Point end=footPrintList[i+1].m_pos;
        Point dir=ccpNormalize(end-start);
        //
        Point normLeft=Point(-dir.y,dir.x);
        Point normRight=Point(0,0)-normLeft;
        Point LU=start+ccpMult(normLeft, R);
        Point LD=start+ccpMult(normRight, R);
        Point RU=end+ccpMult(normLeft, R);
        Point RD=end+ccpMult(normRight, R);
        //  LU----RU
        //   |               |
        // start--end
        //   |               |
        ...
        upQuad.m_posRU=RU;
        upQuad.m_posRD=end;
        upQuad.m_texCoordLU=Point(curLengthFoe/totalLength,0);
        upQuad.m_texCoordLD=Point(curLengthFoe/totalLength,0.5);
        upQuad.m_texCoordRU=Point(curLength/totalLength,0);
        upQuad.m_texCoordRD=Point(curLength/totalLength,0.5);
        ...
        dnQuad.m_posRU=end;
        dnQuad.m_posRD=RD;
        dnQuad.m_texCoordLU=Point(curLengthFoe/totalLength,0.5);
        dnQuad.m_texCoordLD=Point(curLengthFoe/totalLength,1);
        dnQuad.m_texCoordRU=Point(curLength/totalLength,0.5);
        dnQuad.m_texCoordRD=Point(curLength/totalLength,1);
    ...
    for(int i=0;i<nQuad-1;i++){
        ...
        Point leftVec=ccpNormalize(upQuad.m_posLD-upQuad.m_posRD);
        Point rightVec=ccpNormalize(upQuadn.m_posRD-upQuadn.m_posLD);
        ...
        if(turnType==eTurnType_left||turnType==eTurnType_straightBack){
            ...
            Point L1=dnQuad.m_posRD-dnQuad.m_posRU;
            Point L2=dnQuadn.m_posLD-dnQuadn.m_posLU;
            vector<Point> LList;//L1,...,L2
            LList.push_back(L1);
            const float dA=30;
            float A=0;
            while (1) {
                A+=dA;
                Point L=Point(L1.x*cosf(A*M_PI/180)-L1.y*sinf(A*M_PI/180),
            ...
            for(int j=0;j<nQuadMakeUp;j++){
                ...
                quad.m_texCoordLU=Point(dnQuad.m_texCoordRU.x,0.5);
                quad.m_texCoordRU=Point(dnQuad.m_texCoordRU.x,0.5);
                quad.m_texCoordLD=Point(_sa+j*(_sb-_sa)/nQuadMakeUp,1);
                quad.m_texCoordRD=Point(_sa+(j+1)*(_sb-_sa)/nQuadMakeUp,1);
        ...
        if(turnType==eTurnType_right||turnType==eTurnType_straightBack){
            ...
            vector<Cquad> quadList_makeup;
            Point L1=upQuad.m_posRU-upQuad.m_posRD;
            Point L2=upQuadn.m_posLU-upQuadn.m_posLD;
            vector<Point> LList;//L1,...,L2
            LList.push_back(L1);
            const float dA=-30;
            float A=0;
            ...
            for(int j=0;j<nQuadMakeUp;j++){
                Cquad&quad=quadList_makeup[j];
                quad.m_texCoordLU=Point(_sa+j*(_sb-_sa)/nQuadMakeUp,0);
                quad.m_texCoordRU=Point(_sa+(j+1)*(_sb-_sa)/nQuadMakeUp,0);
                quad.m_texCoordLD=Point(upQuad.m_texCoordRD.x,0.5);
                quad.m_texCoordRD=Point(upQuad.m_texCoordRD.x,0.5);
        ...
        if(turnType==eTurnType_left&&isSharp==false){
            Point upMid=ccpMidpoint(upQuad.m_posRU, upQuadn.m_posLU);
            upQuad.m_posRU=upMid;
            upQuadn.m_posLU=upMid;
        }
        
        if(turnType==eTurnType_right&&isSharp==false){
            Point dnMid=ccpMidpoint(dnQuad.m_posRD, dnQuadn.m_posLD);
        ...
        for(int j=0;j<nQuad;j++){
            ...
            if(PND1*PND2<0&&quad.m_isTriangle==false){//self-intersect
                ...
                float opacityP;
                {
                    ...
                    texCoordP=Point((quad.m_texCoordLU.x+quad.m_texCoordRU.x)/2,quad.m_texCoordLU.y+0.5*dis_PtoUp/(dis_PtoUp+dis_PtoDn));
